<div>
    <ui:callout icon="info-circle" color="blue" text="القسم الرئيسي للصفحة، يعمل على لفت انتباه الزائر بعنوان تسويقي جذاب لدفعه لإتخاذ إجراء وكسب انطباع ايجابي." />
    ...
    <div class="mt-5 max-w-3xl">
        <form wire:submit="save" id="header-form" class="flex flex-col gap-y-1">
            ...
            @if($active)
                ...
                <ui:textarea name="text" label="العنوان الفرعي" placeholder="عنوان فرعي يضيف ويشرح العنوان الأساسي"   />
...
new class extends \Livewire\Volt\Component {
    ...
    public function mount() {
        $this->block =  Block::firstOrCreate(['name'=> 'about']);    

        $this->title = data_get($this->block, 'config.title.' . app()->getLocale());
        $this->text = data_get($this->block, 'config.text.' . app()->getLocale());
    ...
    public function save($showMessage = true) {   
        ...
        $this->block->config->set('text.' . app()->getLocale(), $this->text);
