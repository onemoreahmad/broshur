            ...
            @if(is_array($file) && count($file) > 0)
                @foreach($file as $key => $f)
                    <div class="py-3 flex {{ !$loop->last ? 'border-b border-gray-200' : '' }}">
                        <div class="w-16 me-4 flex-shrink-0 shadow-xs rounded-lg" >
            ...
            @else
                @if($file)
                    <div class="mt-3 flex">
                        <div class="w-16 me-4 flex-shrink-0 shadow-xs rounded-lg">
                            @if(is_string($file))
                                <img src="{{ $file }}" class="w-full h-full absoluteX object-cover rounded-lg">
                            @elseif(!is_string($file))
                                @if(collect(['jpg', 'png', 'jpeg', 'webp'])->contains($file->getClientOriginalExtension()))
                        ...
                        <div>
                            ...
                            @if(is_string($file))
                                <div class="text-sm font-medium truncate w-40 md:w-auto">{{ \Str::of($file)->explode('/')->last()  }}</div>
                            @elseif(!is_string($file))
                            ...
                            <div class="flex items-center space-x-1">
                                ...
                                <div class="text-gray-400 text-xs">&bull;</div>
                                @if(is_string($file))
                                    <div class="text-xs text-gray-500 uppercase">{{ \Str::of($file)->explode('.')->last() }}</div>
                                @elseif(!is_string($file))
                            ...
                            <button
                                wire:loading.attr="disabled" 
                                type="button" 
                                x-on:click.prevent="$wire.set('{{ $attributes->wire('model')->value }}', null)" class="text-xs text-red-500 appearance-none hover:underline">
                                حذف
                            </button>
                        </div>
                    </div>
